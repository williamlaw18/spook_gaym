...
1. **Foundation (soul)** — combine soul-like resources (found in the
   caverns, tied to the demon/corruption fiction) to form the foundation
   being. The foundation establishes the monster's core nature — what it
   fundamentally *is* before anything is layered onto it.
2. **Characteristics (worldly resources)** — layer combinations of ordinary
   worldly resources (mundane cavern/surface materials the player uncovers
   over time) onto the foundation. These determine the monster's physical
...
A finished monster is therefore the product of a foundation + a set of
characteristics + (optionally) modifiers, not a single `resource -> monster`
lookup. Two monsters can share a foundation and diverge entirely based on
which characteristics and modifiers were layered on, or vice versa.
...
- The foundation layer likely gates which characteristics/modifiers are even
  compatible (a soul foundation could constrain the possibility space), so
  the system stays combinatorial rather than fully open, avoiding
...
- Whether a foundation can be reused across multiple creation attempts or is
